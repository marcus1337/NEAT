...
std::vector<NEAT*> Selector::selectSingleIDs(std::vector<Specie>& species) {
    ...
    for (int i = 0; i < populationSize; i++) {
        ...
        result.push_back(species[randSpecie].neats[randSpecieIndex]);
    }

    return result;
}

std::vector<std::pair<NEAT*, NEAT*>> Selector::selectPairedIDs(std::vector<Specie>& species) {
    std::vector<int> specieProbabilities = getDiscreteSpecieProbabilities(species);
    for (auto& specie : species)
        specie.setDiscreteProbabilityDistribution();

    std::vector<std::pair<NEAT*, NEAT*>> result;
    for (int i = 0; i < populationSize; i++) {
        int randSpecie = Utils::randIndex(specieProbabilities);
        ...
        std::pair<NEAT*, NEAT*> crossPair;
        crossPair.first = species[randSpecie].neats[randSpecieIndex1];
        crossPair.second = species[randSpecie].neats[randSpecieIndex2];
